...
st.title("🧶 Calculadora de Puntos y Vueltas")
st.write("Calcula las medidas exactas para tus proyectos de tejido.")

st.header("1. Datos de la Muestra")
col1, col2 = st.columns(2)
with col1:
    pts_10cm = st.number_input("¿Cuántos puntos tienes en 10 cm?", min_value=1.0, value=20.0, step=0.5)
with col2:
    vtas_10cm = st.number_input("¿Cuántas vueltas tienes en 10 cm?", min_value=1.0, value=28.0, step=0.5)

pts_por_cm = pts_10cm / 10.0
vtas_por_cm = vtas_10cm / 10.0

st.info(f"Tensión calculada: **{pts_por_cm:.1f} pts/cm** | **{vtas_por_cm:.1f} vtas/cm**")

st.header("2. Medidas de la Prenda")
col3, col4 = st.columns(2)
with col3:
    contorno = st.number_input("¿Qué contorno quieres? (en cm)", min_value=1.0, value=100.0, step=1.0)
with col4:
    largo = st.number_input("¿Qué largo quieres? (en cm)", min_value=1.0, value=60.0, step=1.0)

pts_finales = redondear(contorno * pts_por_cm)
vtas_finales = redondear(largo * vtas_por_cm)

st.success(f"**Puntos necesarios:** {pts_finales} pts\n\n**Vueltas necesarias:** {vtas_finales} vtas")

st.header("3. Medidas Libres (Mangas, Puños, Escotes)")
